...
export const auth = (req, res, next) => {
    ...
    jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => {
        if (err) {
            logError('Auth middleware', 'auth', err, 'Token inválido o expirado', 403);
...
export const requiresAdmin = (req, res, next) => {
    if (!req.user) {
        log('Auth middleware', 'requiresAdmin', 'Usuario no autenticado', null, 401);
    ...
    if (req.user.role !== "admin") {
        log('Auth middleware', 'requiresAdmin', 'Acceso denegado. Se requiere rol de administrador', req.user.role, 403);
        return res.status(403).json({
